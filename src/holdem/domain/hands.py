"""Hand evaluation: find the best 5-card poker hand from 7 cards.
...
from holdem.domain.cards import Card, HandRank, HandScore, Rank, Suit
...
def select_kickers(cards: list[Card], used: list[Card]) -> list[Card]:
    """Return highest cards not already in *used*, sorted descending."""
    return sorted(
        [c for c in cards if c not in used],
...
def get_cards_by_rank(cards: list[Card], ranks: list[Rank]) -> list[Card]:
    needed: Counter[Rank] = Counter(ranks)
    selected: list[Card] = []
    for card in cards:
...
def flush(cards: list[Card]) -> list[Card] | None:
    suit_counts: Counter[Suit] = Counter(card.suit for card in cards)
    for suit, count in suit_counts.items():
        if count >= 5:
            suited = [card for card in cards if card.suit == suit]
            return sorted(suited, key=lambda c: c.rank.value, reverse=True)
    return None


def straight(cards: list[Card]) -> list[Card] | None:
    seen: dict[int, Card] = {}
...
def straight_flush(cards: list[Card]) -> list[Card] | None:
...
def full_house(histogram: dict[Rank, int], cards: list[Card]) -> list[Card] | None:
    ...
    # Additional trips can serve as the pair
    pair_candidates = trips[1:] + find_n_of_a_kind(histogram, 2)
...
def two_pairs(histogram: dict[Rank, int], cards: list[Card], hand: list[Card]) -> list[Card] | None:
...
def find_best_hand(table_cards: list[Card], player_cards: list[Card]) -> HandScore:
    """Return the strongest five-card hand available from the seven cards
    (community + hole).  Evaluation short-circuits from strongest
    ...
    def make(rank: HandRank, chosen: list[Card]) -> HandScore:
        return HandScore(rank, get_high_card_score(chosen), chosen)
